import {
  ...
  id = "arn:aws:iam::454292364057:oidc-provider/app.terraform.io"
}
...
data "aws_iam_policy_document" "terraform_cloud_admin_assume_policy" {
  statement {
    ...
    condition {
      ...
      values   = ["organization:udemy-terraform-course:project:terraform-oidc:workspace:terraform-cli:run_phase:*"]
    }
  }
}
